...
# external_id = the deployment identity. The webhook dedupes on (source,
# external_id) - spec 7 - so a re-run of the same CI job never double-counts.
external_id="${repo}:${run_id}:${run_attempt}"
now="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

# Build the body with printf (no trailing newline) so the bytes we sign are
# byte-for-byte the bytes we send.
payload="$(printf '{"service":"%s","source":"github-actions","external_id":"%s","version":"%s","commit_sha":"%s","reported_deployed_at":"%s"}' \
  "${RELEASEPULSE_SERVICE}" "${external_id}" "${version}" "${sha}" "${now}")"

# HMAC auth (spec 10): sign "<timestamp>.<raw_body>" with the shared secret.
# - timestamp (epoch seconds) lets the server reject stale/replayed requests.
# - event_id = the delivery identity, unique per send (external_id + timestamp),
#   distinct from external_id; the server treats a repeated event_id as a no-op.
ts="$(date +%s)"
event_id="${external_id}:${ts}"
signature="$(printf '%s.%s' "${ts}" "${payload}" \
  | openssl dgst -sha256 -hmac "${RELEASEPULSE_WEBHOOK_SECRET}" | awk '{print $NF}')"
...
code="$(curl -sS -o "${resp}" -w '%{http_code}' \
  ...
  -H "X-Sentinel-Event-Id: ${event_id}" \
